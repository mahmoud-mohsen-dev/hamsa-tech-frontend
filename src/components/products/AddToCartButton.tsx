...
function AddToCartButton({ productInfo }: PropsType) {
  ...
  return (
    ...
    <button
      className={`before:ease relative h-12 ${isLoading && stock > 0 ? 'border-green-300 bg-green-300 hover:shadow-green-300' : 'border-green-500 bg-green-500 hover:shadow-green-500'} w-full overflow-hidden rounded-md border text-base text-white transition-all before:absolute before:-right-[36px] before:top-0 before:h-12 before:w-6 before:bg-white before:opacity-10 before:duration-700 disabled:cursor-not-allowed ${stock > 0 ? 'shadow-[0_5px_25px_-10px_rgb(0,0,0,.1),0_6px_10px_-6px_rgb(0,0,0,.1)] hover:before:right-[calc(100%+36px)] hover:before:-translate-x-full' : 'opacity-60'}`}
      onClick={handleClick}
      dir='ltr'
